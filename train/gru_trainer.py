...
class Trainer:
    def __init__(self, net_config: NetworkConfig, train_config: dict):
        ...
        self.model = torch.nn.DataParallel(Network.from_config(self.net_cfg))
        if self.train_cfg["start_checkpoint"] is not None:
            self.model.load_state_dict(torch.load(self.train_cfg["start_checkpoint"]))

        self.model.cuda()

        train_dataset = MixedDataset(split=self.train_cfg["split"], tokenize=True, 
                                     subsample_lines=True, mode=self.train_cfg["mode"])
        val_dataset = MixedDataset(split=self.train_cfg["test_split"], tokenize=True, 
                                   subsample_lines=True, mode=self.train_cfg["mode"])
    ...
    def validate_epoch(self):
        ...
        for batch, _ in zip(self.val_dataloader, tqdm.trange(1000)):
            ...
            scores.extend(torch.softmax(logits, dim=-1).cpu().numpy().tolist())
    ...
    def train(self):
        for self.epoch in tqdm.trange(self.train_cfg["num_epochs"]):
            ...
            model_path = ARTIFACTS / self.train_cfg["output_dir"] / f"model_{self.epoch}.pth"
